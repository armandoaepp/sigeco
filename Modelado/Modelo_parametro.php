...
public function Insertar_parametro(){
	...
	*/
	$consulta=" CALL sp_ParametroModalidad('$this->ParametroId','$this->ParametroGanulometria17','$this->ParametroGanulometria15','$this->ParametroDefectosPrimarios','$this->ParametroDefectosSecundario','$this->AcopioId')";
...
public function ListarAcopio_parametro(){
	$rpta=array();
	try{
	include_once 'conexion.php';
	$con=new Conexion();
	$consulta= "select * from parametro WHERE  AcopioId='$this->AcopioId'";
	$rpta=$con->Listar($consulta);
	}catch(exception $e){
	 $rpta=$e->getMessage();}
	return $rpta;
