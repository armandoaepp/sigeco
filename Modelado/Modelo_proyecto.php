...
/*
$nombim=$HTTP_POST_FILES["imagen"]["name"];
$temporal=$HTTP_POST_FILES["imagen"]["tmp_name"];
$rutadestino="imagenes/".$nombim;
echo $temporal."<br>";
echo $rutadestino;

if(move_uploaded_file($temporal,$rutadestino)){
	
$sql="insert into producto values(null,'$np',$pre,'$fec',$stock,'$rutadestino',$marca,$cate)";
	$re=$con->consulta($sql);
	if($re){echo "Producto Agregado Correctamente";}	
	}else{
