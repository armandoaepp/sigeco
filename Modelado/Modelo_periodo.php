...
include_once 'conexion.php';
$con=new Conexion();
$consulta="Update periodo set PeriodoEstado='E' where PeriodoId='$this->PeriodoId'";
$rpta=$con->MetodoRegistrar($consulta);
}catch(exception $e){
 $rpta=$e->getMessage();}
return $rpta;
...
include_once 'conexion.php';
$con=new Conexion();
$consulta="Update periodo set PeriodoEstado='A' where PeriodoId='$this->PeriodoId'";
$rpta=$con->MetodoRegistrar($consulta);
}catch(exception $e){
 $rpta=$e->getMessage();}
return $rpta;
...
//Método Buscar Periodo Activo
public function PeriodoActivo_periodo(){
	$rpta=array();
	try{
	include_once 'conexion.php';
	$con=new Conexion();
	$consulta= "SELECT * FROM periodo p WHERE p.PeriodoEstado='A'";
	$rpta=$con->Listar($consulta);
	}catch(exception $e){
	 $rpta=$e->getMessage();}
	return $rpta;
