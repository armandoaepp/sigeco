...
case "Mactualizar":
 ...
 require_once "../layout.php";
break;
case "ListarParcelaAcopio":
	session_start();
	$idAcopio=$_SESSION["idAcopio"];
	if($_GET["editar"]=="Editar")
	$data=CParcelaUnidadProductiva_trazabilidad($idAcopio);	
	else $data=CParcelaAcopio_trazabilidad($idAcopio);
	
	// $TrazabilidadId=$rptamo["cuerpo"][0]["TrazabilidadId"];
	
	$cantele=count($data["cuerpo"]);
	include_once("Listar_Parcela_trazabilidad.php");
break;
}
ob_flush();
?>